...
impl Parse for KernelArgs {
    fn parse(input: ParseStream) -> Result<Self> {
        ...
        Ok(Self {
            // xptr: parse_ident!(args, input),
            // yptr: parse_ident!(args, input),
...
pub fn lmult_upper_tri(
    ...
    let hval = emitters::init_var(&hid, &quote! { #m.min(#p) });
    let mask_m = emitters::name("mask_m");
    let mask_n = emitters::name("mask_n");
    // instructs
    let load_mask_m = emitters::init_var(&mask_m, &quote! { MASK[#m] });
...
pub fn rmult_lower_tri(
    ...
    let load_mask_m = emitters::init_var(&mask_m, &quote! { MASK[#m] });
    let load_mask_n = emitters::init_var(&mask_n, &quote! { MASK[#n] });
    let lfilter = emitters::init_var(&mask_t, &quote! { [0;#i] });
    let tvecs = emitters::mload_vecs(&mask_m, &tids, &tptr, &s_t, i);
    let yvecs = emitters::mload_vecs(&mask_n, &yids, &yptr, &s_y, k);
    let yinc = emitters::increment(&yptr, &s_y, k, 0);
    let xinc = emitters::increment(&xptr, &s_x, 0, k);
...
pub fn rmult_upper_tri(
    ...
    let load_thresh = emitters::init_var(&thresh, &quote! { #m.min(#p) });
    let load_tvecs = emitters::mload_vecs(&mask_m, &tids, &tptr, &s_t, i);
    let load_yvecs = emitters::mload_vecs(&mask_n, &yids, &yptr, &s_y, k);
    let inc_yptr = emitters::increment(&yptr, &s_y, k, 0);
    let inc_xptr = emitters::increment(&xptr, &s_x, 0, k);
    let exec_fma = emitters::mfma_product(&mask_m, &tids, &yids, &xptr, &s_x, i, k);
    let exec_tail = emitters::mhandle_tail(
        &mask_m, &mask_n, &tids, &yids, &xptr, &yptr, &s_x, &s_y, &p, k,
    );
    let exec_tri = emitters::rhandle_uptri(
        &mask_n, &mask_t, &tids, &xptr, &yptr, &s_x, &s_y, &thresh, &yids[0],
    );
    let save_tvecs = emitters::mwrite_outcome(&mask_m, &mask_n, &tids, &tptr, &s_t, i);
    quote! {
        unsafe {
            #load_mask_m
            #load_mask_n
